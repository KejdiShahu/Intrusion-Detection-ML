...
class IoTEda:
    ...
    def run_integrated_analysis(self, df, output_dir='./results/eda'):
        ...
        df['Is_Attack'] = df['Attack_type'].apply(lambda x: 0 if x in normal_classes else 1)
        ...
        top_features = ['flow_duration', 'fwd_pkts_tot', 'bwd_pkts_tot',
                        'fwd_pkts_payload.avg', 'bwd_pkts_payload.avg',
                        'flow_pkts_payload.avg', 'fwd_iat.avg', 'bwd_iat.avg',
                        'idle.avg', 'idle.std', 'bwd_init_window_size']
        ...
        plt.title("Detailed Class Distribution\n(Green = Normal | Red = Attack)", fontsize=14, fontweight='bold')
        plt.xticks(rotation=45)
        plt.ylabel("Count")
        for bar in bars:
            yval = bar.get_height()
            plt.text(bar.get_x() + bar.get_width() / 2, yval + 500, f'{int(yval):,}', ha='center', va='bottom', fontsize=9)
        plt.tight_layout()
        ...
        df[top_features[:9]].hist(bins=30, figsize=(15, 10), color='#3498db', edgecolor='black')
        plt.suptitle("Univariate Analysis: Feature Distribution Grid", fontsize=16, fontweight='bold')
        ...
        for i, col in enumerate(top_features[:6], 1):
            plt.subplot(3, 2, i)
            sns.boxplot(x='Is_Attack', y=col, data=df, palette='Set2', hue='Is_Attack', legend=False)
        ...
        sns.scatterplot(data=df, x='fwd_pkts_payload.avg', y='bwd_pkts_payload.avg', hue='Is_Attack', alpha=0.4,
                        palette='viridis')
        plt.title("Scatter Plot: Forward vs Backward Payload (Traffic Asymmetry)", fontsize=14)
        plt.savefig(f'{output_dir}/eda_5_scatter.png', dpi=150)
        plt.close()

        # -- Page 6: Outlier Visualization (Violin Plots - Point 5)
        plt.figure(figsize=(12, 6))
        sns.violinplot(x='Attack_type', y='flow_duration', data=df, palette='muted', hue='Attack_type', legend=False)
        plt.yscale('log')
        plt.xticks(rotation=45)
        ...
        plt.title("Correlation Heatmap — Key Network Features", fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(f'{output_dir}/eda_7_correlation.png', dpi=150)
        plt.close()

        # -- Page 8: Skewness Report (Numerical Analysis - Point 2)
        skew_vals = df[top_features].apply(lambda x: skew(x.dropna())).sort_values(ascending=False)
