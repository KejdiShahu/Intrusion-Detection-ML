...
class IoTEvaluator:
    ...
    def get_summary(self) -> pd.DataFrame:
        rows = [{k: v for k, v in r.items() if k != "y_pred"}
    ...
    def plot(self, y_test, feature_importances: pd.Series = None,
             loss_curve: list = None):

        fig = plt.figure(figsize=(20, 18))
        gs  = gridspec.GridSpec(3, 3, figure=fig, hspace=0.5, wspace=0.4)
        fig.suptitle("Phase 3 — Model Evaluation", fontsize=16,
        ...
        for i, (r, col) in enumerate(zip(self.results, colors)):
            ax4.bar(x + i*w, [r[m] for m in metrics], w,
                    label=r["Model"], color=col, alpha=0.85)
        ax4.set_xticks(x + w)
        ...
        if loss_curve is not None:
            ...
            ax6.set_title("Neural Network — Loss Curve",
                          fontweight="bold")
        ...
        ax7.axis("off")
        data = [[r["Model"], f"{r['Accuracy']:.4f}",
                 f"{r['F1']:.4f}", f"{r['ROC-AUC']:.4f}"]
                for r in self.results]
        tbl = ax7.table(cellText=data,
